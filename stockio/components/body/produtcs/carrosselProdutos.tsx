...
interface Props {
  titulo?: string;
  subtitulo?: string;
  categoriaFiltro?: string; // usado para “ver mais”
}
  ...
  useEffect(() => {
    async function fetchProdutos() {
      try {
        ...
        setProdutos(response.data.slice(0, 15));
    ...
    <section className="w-full px-6 py-8">
      ...
      <div className="flex items-center justify-between px-4 mt-6">
        ...
        <h2 className="text-3xl font-semibold md:text-5xl text-black mb-4">
          {titulo}
          <span className="ml-2 text-sm font-normal text-[#5E3C9E]">
            em {subtitulo}
          </span>
        </h2>
        ...
        {/* 👉 VER MAIS */}
        <Link
          href={`/categorias/${categoriaFiltro}`}
          className="text-[#5E3C9E] hover:underline text-md font-medium"
        >
          ver mais
        </Link>
      ...
      {isLoading ? (
        <div className="flex gap-8 mt-4 overflow-hidden">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="shrink-0 w-56 h-80 bg-gray-200 rounded-lg animate-pulse"
            />
          ))}
        </div>
      ) : (
